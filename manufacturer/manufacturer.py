...
from order import business_order
...
class Manufacturer:
    ...
    def check_stock(self, rm1, rm2, rm3):
        if rm1 < self.stock_1.get_inventory():
            ...
            self.env.process(self.initialize_business_order(deviation_1, self.stock_1.get_material_type()))
        elif rm2 < self.stock_2.get_inventory():
            deviation_2 = rm2 - self.stock_2.get_inventory()
            self.env.process(self.initialize_business_order(deviation_2, self.stock_2.get_material_type))
        elif rm3 < self.stock_3.get_inventory():
            deviation_3 = rm3 - self.stock_3.get_inventory()
            self.env.process(self.initialize_business_order(deviation_3, self.stock_3.get_material_type()))
        return True

    def initialize_business_order(self, deviation, material_type):
        order = business_order.BusinessOrder(quantity=deviation,
